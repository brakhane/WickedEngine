...
#define xaudio_assert(cond, fname)									  \
{																	  \
	std::string s;													  \
	wilog_assert(cond,												  \
		s.append("XAudio2 error: ")									  \
		 .append(fname)												  \
		 .append(" failed with %s (%s:%d)")							  \
		 .c_str(),													  \
		wi::helper::GetPlatformErrorString(hr),						  \
		relative_path(__FILE__),									  \
		__LINE__													  \
	);																  \
}

#define xaudio_check(call)											  \
[&]() {																  \
	HRESULT hr = call;												  \
	xaudio_assert(SUCCEEDED(hr), extract_function_name(#call));		  \
		return hr;													  \
}()
